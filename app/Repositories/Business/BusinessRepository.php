...
{
    public function findBusinessBySlug(string $slug) 
    {
        return Business::select('id','name', 'slug')
                            ->where('slug',  $slug)
                            ->first(); 
    } 
    public function findBusinessDetailsBySlug(Business $business) 
    {
        $query = Business::with([ 
                'locations:id,business_id,address,city,country',
        ...
        return new BusinessDetailsResource($query);
    }
    public function findBusinessTypes(Business $business)
    {
        return BusinessType::select('id','name')->where('language_id', $business->language_id)->get();
    }
